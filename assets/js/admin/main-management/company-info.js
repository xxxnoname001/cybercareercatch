document.addEventListener("DOMContentLoaded", function(){
    ...
    const deleteBtn = document.querySelector(".qnaMgmt-del button");
    const allRows = Array.from(document.querySelectorAll(".qnaMgmt-row"));
    ...
    deleteBtn.addEventListener("click", function(){
        ...
        const checkedBoxes = visibleRows.map(row => row.querySelector("input[type='checkbox']")).filter(box => box.checked);
        ...
        if(confirm("선택한 항목을 삭제하시겠습니까?")){
            checkedBoxes.forEach(box => {
                const index = allRows.indexOf(box.closest(".qnaMgmt-row"));
